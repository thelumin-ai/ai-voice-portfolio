...
interface RetellVoiceDemoProps {
    agentRole?: string;
    agentId?: string;
    apiKey?: string;
}

export default function RetellVoiceDemo({ agentRole = 'AI Agent', agentId, apiKey }: RetellVoiceDemoProps) {
    ...
    useEffect(() => {
        ...
        client.on("update", (update: any) => {
            // Retell transcript update format varies slightly by version, 
            // usually it gives the full transcript array in update.transcript
            if (update && update.transcript) {
                const formattedMessages = update.transcript.map((msg: any) => ({
                    role: msg.role === 'agent' || msg.role === 'assistant' ? 'agent' : 'user',
                    content: msg.content
                }));
                setMessages(formattedMessages);
                
                // If the last message was user, we might be processing
        ...
        client.on("error", (err: any) => {
            ...
            setState('idle');
            setError("Connection error: " + (err.message || "Failed to connect to Retell."));
        });
    ...
    const startConversation = async () => {
        if (!agentId) {
            setError("No Retell Agent ID provided.");
            return;
        }
        ...
        try {
            ...
            // Extract pure ID in case the user pasted a full URL by accident
            let parsedAgentId = agentId;
            if (agentId.includes('http')) {
                 const urlParams = new URLSearchParams(new URL(agentId).search);
                 parsedAgentId = urlParams.get('agent_id') || urlParams.get('id') || agentId.split('/').pop() || agentId;
            }

            // 1. Fetch access token from our proxy route
            const response = await fetch('/api/retell/create-web-call', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ agentId: parsedAgentId, apiKey })
            });
            ...
            <div ref={transcriptRef} className="flex-1 p-6 overflow-y-auto space-y-4 scroll-smooth z-10 bg-white/50 dark:bg-transparent">
                {error ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 space-y-4">
                        ...
                        <p>Ready to start the Retell conversation.</p>
                    </div>
                ) : (
                    <>
                        {messages.map((msg, i) => (
                ...
                <AnimatePresence mode="wait">
                    ...
                    {state === 'connecting' && (
                        <motion.div key="connecting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-yellow-500 flex items-center">
                            <Loader2 className="w-5 h-5 mr-3 animate-spin" /> <span>Connecting Retell...</span>
                        </motion.div>
                    )}
                    {state === 'listening' && (
